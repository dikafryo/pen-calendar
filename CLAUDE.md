...
Recurrence logic lives entirely in the renderer (`parseRrule` / `buildRrule` / `expandRruleDates` / `expandRecurrencesForRange`). Supported subset: `FREQ=DAILY|WEEKLY|MONTHLY|YEARLY`, `INTERVAL`, `COUNT`, `UNTIL`, single `BYDAY` token on `MONTHLY` (e.g. `3TH`, `-1FR` → `r.byday = {ordinal, dow}`), and multi-`BYDAY` (no ordinals) on `WEEKLY` (e.g. `MO,WE,FR` → `r.bydays = [1,3,5]`). MONTHLY multi-BYDAY (`1MO,3MO`), `BYMONTHDAY`, and `BYSETPOS` are **not** supported — extending requires changes here, not in `sync/*`. WEEKLY+BYDAY auto-includes the master's start-day so the master date is always the first occurrence. NextCloud round-trips RRULE/EXDATE through ICAL.js; Google currently does **not** push recurrence (events are pushed as singletons). Detached instances are stored as separate events with `recurrenceId` pointing at the master; `originalMasterTime` must be preserved on detach so NextCloud can match by RECURRENCE-ID.
